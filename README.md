...
- **`appconfig/markdown-config.jsonc` 控制渲染外观**：公式块圆角背景、列表 `·` 大小描边、代码块整块圆角、引用 / 链接 / 标题倍率 / 分割线等，FileObserver 热更（JSONC 格式，默认模板 + 用户覆盖深合并）
- **AI 可编排前端效果**：让 DSH 用 write 工具改 JSONC 配置，回到聊天页即时生效，无需重编译或重启 App（真机验证通过）
