...
## 八、M3 细化规划：体验优化 + 文件管理 + 真终端（2026-08-11 主人指示）

> 主人拍板：**先安排优化软件体验，开发文件管理和 termux（真终端）。知识库后面再说。**
...
3. ✅ 真终端（termux 化）：持久 bash PTY，自由切目录、执行代码、交互程序（vim/top）可用
...
| # | 风险 | 对策 |
|---|---|---|
| 1 | PTY + 转义序列渲染复杂度高 | 先验证 termux-terminal-view 可用性；不行就自绘 ANSI 子集 |
| 2 | SELinux 限制真终端 exec 二进制 | node 类操作走 pi RPC bash（linker64 包装可行）；普通命令不受限 |
| 3 | 近似匹配搜索库选择 | 轻量自研（fzy 算法移植）避免体积膨胀 |
| 4 | 文件访问范围 | App 私有目录 + SAF（Storage Access Framework）访问用户目录，不申请存储权限（Android 11+ 分区存储） |
